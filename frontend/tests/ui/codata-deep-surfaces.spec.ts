...
test.describe("Codata deep claimed-feature GUI surfaces", () => {
  ...
  test("messaging channel workflow: supported platforms and disconnect-before-edit path", async ({ page }) => {
    const state = await setupMockedApp(page);
    ...
    await expect(page.getByText("QQ")).toBeVisible();
    await expect(page.getByText("Telegram")).toBeVisible();

    const telegramCard = page.locator("div.rounded-lg").filter({ hasText: "Telegram" }).first();
    ...
    await telegramCard.getByRole("button", { name: "Disconnect" }).click();
    await expect.poll(() => JSON.stringify(state.channelRemoves)).toContain("telegram");
    await expect(telegramCard.getByRole("button", { name: "Edit" })).toBeVisible();
  ...
  test("standalone and first-run workflows: routes render outside settings and onboarding routes to providers", async ({ page }) => {
    ...
    await expect(page.locator("h1", { hasText: "Messaging Channels" })).toBeVisible();
    await expect(page.getByText("DingTalk")).toBeVisible();
    await expectNoAppCrash(page);
