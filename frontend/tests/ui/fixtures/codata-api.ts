...
export async function mockCodataApi(page: Page, options: CodataMockOptions = {}): Promise<CodataMockState> {
  const state: CodataMockState = {
    ...
    modelRefreshes: [],
    dashboardItems: [],
    dashboards: [{ id: "dash-default", name: "我的看板", is_default: true, position: 0, item_count: 0, time_created: "2026-04-21T10:00:00.000Z" }],
  };
  ...
  const handler = async (route: Route) => {
    ...
    if (path === "/api/dashboards" && method === "GET") {
      const withCounts = state.dashboards.map((b) => ({
        ...b,
        item_count: state.dashboardItems.filter((i) => i.dashboard_id === b.id).length,
      }));
      return fulfillJson(route, withCounts);
    }
    if (path === "/api/dashboards" && method === "POST") {
      ...
      const board = {
        ...
        name: typeof body.name === "string" ? body.name : "未命名看板",
        is_default: false,
        position: state.dashboards.length,
        item_count: 0,
        time_created: "2026-04-21T12:00:00.000Z",
      };
      state.dashboards.push(board);
      return fulfillJson(route, board);
    }
    if (path.startsWith("/api/dashboards/") && method === "PATCH") {
      const id = decodeURIComponent(path.split("/").pop() ?? "");
      const body = (requestJson(request) ?? {}) as Record<string, unknown>;
      const board = state.dashboards.find((b) => b.id === id);
      if (board && typeof body.name === "string") board.name = body.name;
      return fulfillJson(route, board ?? {});
    }
    if (path.startsWith("/api/dashboards/") && method === "DELETE") {
      const id = decodeURIComponent(path.split("/").pop() ?? "");
      if (state.dashboards.length > 1) {
        state.dashboards = state.dashboards.filter((b) => b.id !== id);
        state.dashboardItems = state.dashboardItems.filter((i) => i.dashboard_id !== id);
      }
    ...
    if (path === "/api/dashboard/items" && method === "GET") {
      const dashId = url.searchParams.get("dashboard_id");
      const items = dashId
        ? state.dashboardItems.filter((i) => i.dashboard_id === dashId)
        : state.dashboardItems;
      return fulfillJson(route, items);
    }
    if (path === "/api/dashboard/items" && method === "POST") {
      const body = (requestJson(request) ?? {}) as Record<string, unknown>;
      const defaultBoard = state.dashboards.find((b) => b.is_default) ?? state.dashboards[0];
      const item = {
        id: `dash-${state.dashboardItems.length + 1}`,
        dashboard_id:
          typeof body.dashboard_id === "string" ? body.dashboard_id : defaultBoard?.id ?? null,
    ...
    if (path === "/api/dashboard/layout" && method === "POST") {
      return fulfillJson(route, { success: true });
    }
    ...
    if (path === "/api/channels") {
      return fulfillJson(route, {
        gateway_running: configuredChannels.size > 0,
        channels: Object.fromEntries(
          [...configuredChannels.entries()].map(([id, channel]) => [
            id,
            { id, name: id.charAt(0).toUpperCase() + id.slice(1), status: channel.status, type: channel.type },
          ]),
        ),
      });
    }
    if (path === "/api/channels/weixin/qr/start") {
      weixinQrPolls = 0;
      return fulfillJson(route, {
        session_id: "mock-weixin-qr",
        scan_url: "https://scan.example.com/mock-weixin-qr",
        expires_at: Math.floor(Date.now() / 1000) + 300,
        status: "waiting_scan",
      });
    }
    if (path === "/api/channels/weixin/qr/mock-weixin-qr" && method === "GET") {
      weixinQrPolls += 1;
      if (weixinQrPolls >= 2) {
        configuredChannels.set("weixin", { status: "running", type: "weixin" });
        return fulfillJson(route, {
          session_id: "mock-weixin-qr",
          status: "confirmed",
          message: "Connected",
          account: "mock-weixin-user",
        });
      }
      return fulfillJson(route, {
        session_id: "mock-weixin-qr",
    ...
    if (path === "/api/channels/weixin/qr/mock-weixin-qr/cancel") {
      return fulfillJson(route, { ok: true });
    }
    if (path === "/api/channels/feishu/qr/start") {
      feishuQrPolls = 0;
      return fulfillJson(route, {
        session_id: "mock-feishu-qr",
        scan_url: "https://accounts.feishu.cn/passport/device?user_code=MOCK",
        expires_at: Math.floor(Date.now() / 1000) + 600,
        status: "waiting_scan",
      });
    }
    if (path === "/api/channels/feishu/qr/mock-feishu-qr" && method === "GET") {
      feishuQrPolls += 1;
      if (feishuQrPolls >= 2) {
        configuredChannels.set("feishu", { status: "running", type: "feishu" });
        return fulfillJson(route, {
          session_id: "mock-feishu-qr",
          status: "confirmed",
          message: "Connected",
          account: "mock-feishu-user",
        });
      }
      return fulfillJson(route, {
        session_id: "mock-feishu-qr",
    ...
    if (path === "/api/channels/feishu/qr/mock-feishu-qr/cancel") {
      return fulfillJson(route, { ok: true });
    }
    if (path === "/api/channels/add") {
      const body = requestJson(request) as Record<string, unknown> | null;
      state.channelAdds.push(body);
      const channel = typeof body?.channel === "string" ? body.channel : "telegram";
      configuredChannels.set(channel, { status: "running", type: channel });
      return fulfillJson(route, { ok: true, message: "Connected" });
    }
    if (path === "/api/channels/remove") {
      const body = requestJson(request) as Record<string, unknown> | null;
      state.channelRemoves.push(body);
      const channel = typeof body?.channel === "string" ? body.channel : "telegram";
      configuredChannels.set(channel, { status: "disabled", type: channel });
