...
test.describe("Codata complete GUI workflows", () => {
  ...
  test("messaging channel journey: connect, disconnect, then edit configuration", async ({ page }) => {
    const state = await setupMockedApp(page);
    ...
    await clientIdInput.fill("ding-client");
    await page.getByPlaceholder("Enter DingTalk client secret or app secret").fill("ding-secret");
    ...
    await expect.poll(() => JSON.stringify(state.channelAdds)).toContain("ding-client");

    await expect(dingTalkCard.getByRole("button", { name: "Disconnect" })).toBeVisible();
    ...
    await dingTalkCard.getByRole("button", { name: "Edit" }).click();
    const updatedClientIdInput = page.getByPlaceholder("Enter DingTalk client ID or app key");
    await updatedClientIdInput.fill("ding-client-updated");
    await page.getByPlaceholder("Enter DingTalk client secret or app secret").fill("ding-secret-updated");
    const updatedDingTalkForm = updatedClientIdInput.locator("xpath=ancestor::div[contains(@class, 'space-y-2')][1]");
    await updatedDingTalkForm.getByRole("button", { name: "Save & Connect" }).click();
    await expect.poll(() => JSON.stringify(state.channelAdds)).toContain("ding-client-updated");
