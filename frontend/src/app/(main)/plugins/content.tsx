...
export function ConnectorsTab({ search }: { search: string }) {
  const { t } = useTranslation("plugins");
...
export function SkillsTab({ search }: { search: string }) {
  const { t } = useTranslation("plugins");
        ...
        {skills.map((skill) => (
          ...
          >
            <button
              ...
              className={`flex items-start gap-3 min-w-0 flex-1 rounded-md text-left transition-colors hover:bg-[var(--surface-tertiary)] focus:outline-none focus:ring-1 focus:ring-[var(--border-focus)] -m-1 p-1 ${!skill.enabled ? "opacity-50" : ""}`}
              title={t("viewSkill", { defaultValue: "View skill" })}
            >
              <Sparkles className="h-3.5 w-3.5 text-[var(--text-tertiary)] mt-0.5 shrink-0" />
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
              ...
              <Eye className="h-3.5 w-3.5 text-[var(--text-tertiary)] mt-0.5 shrink-0 opacity-70" />
            </button>
            <Switch
