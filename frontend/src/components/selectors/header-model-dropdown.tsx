...
/**
 * Variant keywords that disambiguate provider-owned model tiers
...
export function HeaderModelDropdown() {
  ...
  useEffect(() => {
    ...
    const currentModel = visibleModels.find((m) => modelMatches(m, selectedModel, selectedProviderId));
    if (currentModel) {
      if (selectedProviderId !== currentModel.provider_id) {
        setSelectedModel(currentModel.id, currentModel.provider_id);
      }
    } else {
