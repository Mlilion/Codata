...
export function ContextIndicator({ sessionId }: ContextIndicatorProps) {
  ...
  const isChatCompacting = chatSession.isCompacting;
  ...
  const handleManualCompact = useCallback(async () => {
    ...
    try {
      ...
      useChatStore.getState().startCompactionStream(result.session_id, result.stream_id);
  ...
  }, [isChatCompacting, isGenerating, isStartingCompact, meetsManualCompactionThreshold, queryClient, selectedModel, sessionId, t]);
