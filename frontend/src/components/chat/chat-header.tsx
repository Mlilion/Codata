...
export function ChatHeader({ sessionId, showModelSelector = true }: ChatHeaderProps) {
  ...
  // over the left edge of the chat area, so reserve space.
  ...
  const chatSession = useChatSession(sessionId ?? null);
  const isGenerating = chatSession.isGenerating;
  const streamingParts = chatSession.streamingParts;
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                size="icon"
                className="h-9 w-9"
                aria-label={rightSidebarOpen ? t("hideWorkspace") : t("showWorkspace")}
            ...
            <TooltipContent side="bottom">
              {rightSidebarOpen ? t("hideWorkspace") : t("showWorkspace")}
            </TooltipContent>
          </Tooltip>
        )}
