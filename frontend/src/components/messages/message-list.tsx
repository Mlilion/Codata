...
export function MessageList({
  ...
  const historicalExpertActivity = useMemo(() => {
    if (!sessionId || isGenerating || !!streamId) return null;
    for (let index = groups.length - 1; index >= 0; index -= 1) {
      const group = groups[index];
      if (group.kind !== "assistant") continue;
      if (!group.messages.every((message) => message.session_id === sessionId)) continue;
      const parts = visibleMessageParts(group.messages.flatMap((msg) => msg.parts.map((part) => part.data as PartData)));
      const activity = buildExpertActivityData(sessionId, parts);
      if (activity) return activity;
    }
    return null;
  }, [groups, isGenerating, sessionId, streamId]);

  useEffect(() => {
    const key = historicalExpertActivity?.sourceKey;
    if (!key || restoredExpertActivityKeyRef.current === key) return;
    const frame = requestAnimationFrame(() => {
      restoredExpertActivityKeyRef.current = key;
      openForMessage(key, historicalExpertActivity);
    });
    return () => cancelAnimationFrame(frame);
  }, [historicalExpertActivity, openForMessage]);
...
function buildExpertActivityData(sessionId: string, parts: PartData[]): ActivityData | null {
  ...
  if (!hasExpertSteps) return null;

  const reasoningTexts = parts
    .filter((part): part is PartData & { type: "reasoning" } => part.type === "reasoning")
    .map((part) => part.text);
  const toolParts = parts.filter((part): part is ToolPart => part.type === "tool");
  const chain: ChainItem[] = [];
  for (const part of parts) {
    if (part.type === "reasoning") chain.push({ type: "reasoning", text: part.text });
    else if (part.type === "tool") chain.push({ type: "tool", data: part });
  }
  ...
  const data: ActivityData = {
    ...
    toolParts,
    stepParts,
    hasVisibleOutput: parts.some((part) =>
      part.type === "text" || part.type === "file" || part.type === "subtask",
    ),
    chain,
  };
