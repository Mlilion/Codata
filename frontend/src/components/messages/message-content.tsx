...
import { ExpertTeamTimeline, hasExpertTeamTimeline } from "@/components/parts/expert-team-timeline";
import { ExpertTeamDraftCard, isExpertTeamDraftTool } from "@/components/parts/expert-team-draft-card";
...
function fileCardsForTool(part: ToolPart, presentedFilePaths: Set<string>) {
  ...
  return collectToolFilePaths(part)
    ...
    .map((filePath) => ({
      filePath,
      title: part.tool === "present_file" ? title : undefined,
    }));
}
...
export function MessageContent({
  ...
  const isExpertTeam = hasExpertTeamTimeline(parts);
  ...
  const hasActivity = hasReasoning || hasTools || hasExpertSteps;
  ...
  const activityData = useMemo<ActivityData | null>(
    () =>
      hasActivity
        ...
        : null,
    [hasActivity, hasExpertSteps, reasoningTexts, toolParts, thinkingDuration, stepParts, chain, parts, activityKey],
  );
  ...
  return (
    <div className="space-y-3">
      ...
      {!isExpertTeam && isStreaming && hasActivity && hasMeaningfulActivity && (
        <ReasoningPart
      ...
      {!isExpertTeam && !isStreaming && activityData && <ActivitySummary data={activityData} />}
      ...
      {!isExpertTeam && contentParts.map((part, contentIndex) => {
