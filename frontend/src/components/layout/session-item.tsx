...
import { Archive, Loader2, MessageCircle, Pin, PinOff, Users, type LucideIcon } from "lucide-react";
...
/** Map session slug prefix to a sidebar badge. */
function getSessionBadge(slug: string): { label: string; color: string; Icon: LucideIcon } | null {
  if (!slug) return null;
  const prefix = slug.split(":")[0];
  switch (prefix) {
    case "expert-team": return { label: "专家团", color: "text-[var(--brand-primary)]", Icon: Users };
    case "telegram": return { label: "Telegram", color: "text-sky-400", Icon: MessageCircle };
    case "feishu":   return { label: "Feishu",   color: "text-blue-500", Icon: MessageCircle };
    case "dingtalk": return { label: "DingTalk", color: "text-blue-400", Icon: MessageCircle };
    case "weixin":   return { label: "WeChat",   color: "text-green-500", Icon: MessageCircle };
    case "wecom":    return { label: "WeCom",    color: "text-blue-500", Icon: MessageCircle };
    case "qq":       return { label: "QQ",       color: "text-sky-400", Icon: MessageCircle };
    default: return slug.includes(":") ? { label: prefix, color: "text-[var(--text-tertiary)]", Icon: MessageCircle } : null;
  }
}
