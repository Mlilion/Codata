...
import type { PartData, StepStartPart } from "@/types/message";
    ...
    <div className="overflow-hidden rounded-lg border border-[var(--border-default)] bg-[var(--surface-primary)]">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              ...
              <span className="inline-flex min-h-6 items-center rounded-md bg-[var(--color-success-soft)] px-2 text-ui-3xs font-semibold text-[var(--color-success)]">
      ...
      <div className="relative">
        ...
        {steps.map((step, index) => {
          ...
          const hasDetails = textParts.length > 0 || fileParts.length > 0 || fullSummary.length > 0;
          ...
          return (
            <div
              ...
              className={cn(
                ...
                running && "bg-[var(--brand-soft)] shadow-[inset_3px_0_0_var(--data-accent)]",
              )}
            >
              <div className="grid grid-cols-[42px_minmax(0,1fr)_64px] gap-3 px-4 py-3.5">
                <div className="relative z-[1] flex items-start justify-center">
                  <span className={cn(
                    ...
                    step.status === "completed" && !running && "border-[var(--color-success)]/30",
                    running && "border-[var(--brand-border)]",
                    step.status === "failed" && "border-[var(--color-destructive)]/30",
              ...
              {hasDetails && detailsOpen && (
                <div className="mx-4 mb-3 space-y-3 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-secondary)] p-3 sm:ml-[58px]">
                  <div className="space-y-2">
                    <div className="text-ui-3xs font-semibold text-[var(--text-tertiary)]">
                      {running ? "专家输出（生成中）" : step.handoff ? "专家结论与交接" : "专家完整输出"}
                    </div>
                    {textParts.length > 0 ? (
                  ...
                  {fileParts.length > 0 && (
                    <div className="grid gap-1.5 border-t border-[var(--border-subtle)] pt-3 sm:grid-cols-2">
                      {fileParts.map((file) => (
                        <FileArtifactCard
                          key={file.file_id}
                          filePath={file.path}
