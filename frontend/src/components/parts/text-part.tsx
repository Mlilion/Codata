...
function pathArtifactType(filePath: string): ArtifactType {
  ...
  if (!match) {
    ...
    if (isFilePath) {
      ...
      if (hasPathSegments && canPreview) {
        ...
        const handleOpen = () => {
          const type = pathArtifactType(text);
          if (existing) {
            openArtifact({ ...existing, type, filePath: existing.filePath || text });
          } else {
            openArtifact({
              ...
              type,
              content: "",
              language: type === "code" ? languageFromExtension(text) : undefined,
              filePath: text,
            });
          }
        };
      ...
      if (matchingFiles.length === 1 && isPreviewableFile(matchingFiles[0].path)) {
        ...
        const handleOpenTrackedFile = () => {
          const type = pathArtifactType(matchedFile.path);
          openArtifact({
            id: `tracked-file-preview-${matchedFile.path}`,
            title: matchedFile.name,
            type,
            content: "",
            language: type === "code" ? languageFromExtension(matchedFile.path) : undefined,
