 ...
 * PyInstaller spec silently filtered missing paths, so bundled static
 * assets returned 404. Never let that happen again.
 */
...
const required = [
  ...
  {
    kind: "nonempty-dir",
    path: join(internal, "app", "expert", "presets"),
    why: "expert team presets",
  },
  ...expertPresetFiles.map((filename) => ({
    kind: "file",
    path: join(internal, "app", "expert", "presets", filename),
    why: `expert team preset: ${filename}`,
  })),
  ...
  // Frontend static export — the backend serves root static assets and
  // Next.js chunks from this copy while Tauri serves the main desktop UI
  // from its own resources.
  {
    kind: "dir",
    path: join(internal, "frontend_out"),
    why: "frontend static export",
  },
  {
    kind: "file",
...
// probe /health, then kill it. If it crashes with a missing import we
// catch it here, not in the wild.
//
...
async function smokeTest(bin, port) {
  ...
  if (crashed && !ok) {
    console.error(
      `\n[verify-bundle] smoke FAILED: backend exited (code ${crashCode}) before serving /health`,
    );
  ...
  if (!ok) {
    console.error("\n[verify-bundle] smoke FAILED: /health never returned status ok");
    console.error("--- last backend output ---");
    console.error(logs.join("").slice(-4000));
    exit(1);
  }

  console.log("[verify-bundle] smoke: /health served successfully - bundle is live");
}
