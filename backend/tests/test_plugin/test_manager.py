...
def _make_meta(name: str, *, default_enabled: bool = False) -> PluginMeta:
    return PluginMeta(
        name=name,
        version="1.0",
        description="test",
...
def _make_load_result(
    ...
    *,
...
) -> tuple[PluginLoadResult, dict[str, PluginMeta]]:
    ...
    meta_map = {plugin_name: _make_meta(plugin_name, default_enabled=default_enabled)}
...
class TestPluginManager:
    ...
    def test_manifest_default_enabled_keeps_skills_registered(self, tmp_path: Path):
        sr = SkillRegistry()
        sr.register(_make_skill("myplugin:greet"))
        pm = PluginManager(sr, project_dir=str(tmp_path))
        result, meta_map = _make_load_result("myplugin", default_enabled=True)
        pm.register_loaded(result, "builtin", meta_map)
        assert sr.get("myplugin:greet") is not None
        detail = pm.detail("myplugin")
    ...
    def test_disabled_override_beats_manifest_default_enabled(self, tmp_path: Path):
        sr = SkillRegistry()
        sr.register(_make_skill("myplugin:greet"))
        pm = PluginManager(sr, project_dir=str(tmp_path))
        result, meta_map = _make_load_result("myplugin", default_enabled=True)
        ...
        sr2.register(_make_skill("myplugin:greet"))
        pm2 = PluginManager(sr2, project_dir=str(tmp_path))
        result2, meta_map2 = _make_load_result("myplugin", default_enabled=True)
        pm2.register_loaded(result2, "builtin", meta_map2)
        detail = pm2.detail("myplugin")
        assert detail is not None
        assert detail["enabled"] is False
        assert sr2.get("myplugin:greet") is None
