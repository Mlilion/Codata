...
class TestRuntimeOrigin:
    """Runtime origins cannot be baked into the static allowlist or the
    env-var override; the middleware must read a mutable runtime set from
    app.state."""

    @pytest.mark.asyncio
    async def test_active_runtime_origin_allowed(self):
        """Once a runtime URL is registered on app.state, browser POSTs
    ...
    @pytest.mark.asyncio
    async def test_unrelated_dynamic_origin_still_blocked(self):
        """Only the currently registered runtime URL is trusted."""
        app = _make_app()
        app.state.runtime_allowed_origins = {"https://abc-xyz.trycloudflare.com"}
        async with await _client(app) as c:
            r = await c.post(
    ...
    @pytest.mark.asyncio
    async def test_runtime_origin_eviction_takes_effect_immediately(self):
        """Removing a URL from the runtime set blocks it on the very next
        request — no backend restart, no middleware rebuild."""
        app = _make_app()
        app.state.runtime_allowed_origins = {"https://abc-xyz.trycloudflare.com"}
        async with await _client(app) as c:
            r = await c.post(
            ...
            assert r.status_code == 200, r.text
            # Swap the runtime allowlist entry.
    ...
    async def test_trailing_slash_tolerated(self):
        """Dynamic origin URLs may be registered with or without a trailing
