...
class TestReadTool:
    ...
    @pytest.mark.asyncio
    async def test_read_attached_file_outside_workspace(self, tool: ReadTool, tmp_path: Path):
        workspace = tmp_path / "workspace"
        workspace.mkdir()
        upload_dir = tmp_path / "uploads"
        upload_dir.mkdir()
        attached = upload_dir / "attached.txt"
        attached.write_text("uploaded attachment content\n")
        ...
        ctx = ToolContext(
            ...
            allowed_file_paths={str(attached.resolve())},
        )

        result = await tool.execute({"file_path": str(attached)}, ctx)
        assert result.success
        assert "uploaded attachment content" in result.output
