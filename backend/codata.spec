...
dingtalk_stream_datas, dingtalk_stream_binaries, dingtalk_stream_hiddenimports = collect_all('dingtalk_stream')
telegram_datas, telegram_binaries, telegram_hiddenimports = collect_all('telegram')
lark_oapi_datas, lark_oapi_binaries, lark_oapi_hiddenimports = collect_all('lark_oapi')
botpy_datas, botpy_binaries, botpy_hiddenimports = collect_all('botpy')
wecom_aibot_datas, wecom_aibot_binaries, wecom_aibot_hiddenimports = collect_all('wecom_aibot_sdk')
...
# the spec aborts instead of silently shipping a broken bundle. Never weaken this
...
_required_datas = [
    ...
    # Expert team presets
    (os.path.join(app_dir, 'expert', 'presets'), os.path.join('app', 'expert', 'presets')),
    # Frontend static export. Tauri reads the main frontend from its own
    # resources; the backend bundle still includes the export for routes
...
if _missing:
    ...
    sys.stderr.write(
        ...
        'backend/alembic, backend/app/agent/prompts, backend/app/data and\n'
        'backend/app/expert/presets all\n'
        'exist before running pyinstaller. Aborting so we never ship a\n'
        'half-baked bundle.\n'
    )
    raise SystemExit(1)

# Sanity-check that the frontend export actually contains Next.js assets. A
# stale `frontend/out` from a non-desktop build would otherwise slip past the
...
a = Analysis(
    ...
    binaries=(
        ...
        + wcmatch_binaries
        + croniter_binaries
        + dingtalk_stream_binaries
        + telegram_binaries
        + lark_oapi_binaries
        + botpy_binaries
        + wecom_aibot_binaries
    ),
    datas=(
        datas
        + uvicorn_datas
    ...
    hiddenimports=(
        ...
        + dingtalk_stream_hiddenimports
        + telegram_hiddenimports
        + lark_oapi_hiddenimports
        + botpy_hiddenimports
        + wecom_aibot_hiddenimports
    ),
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
