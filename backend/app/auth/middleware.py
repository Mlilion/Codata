"""Bearer-token authentication middleware for the Codata local API.

Threat model
------------

Codata's FastAPI backend binds to ``127.0.0.1`` on an ephemeral port. The
...
session token lives in a 0600 file that only the Codata-running user
...
The session token is rotated on backend startup, held in
``app.state.session_token``, and never leaves the host filesystem.
...
from app.auth.token import validate_token
...
class AuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # distinguish local from non-local callers if needed.
    ...
    def _token_matches(provided: str, session_token: str, settings) -> bool:
        """Constant-time match against the current session token."""
        return validate_token(provided, session_token)

    @staticmethod
    def _client_ip(scope, headers: dict[bytes, bytes]) -> str:
        # X-Forwarded-For is attacker-controlled for loopback desktop
        # traffic, so rate limiting keys on the socket peer.
    ...
    async def _reject(send, status: int, detail: str) -> None:
        ...
        await send({
            ...
            "headers": [
                ...
                [b"www-authenticate", b'Bearer realm="codata"'],
            ],
        })
        await send({
