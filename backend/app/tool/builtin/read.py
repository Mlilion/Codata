...
def _is_allowed_attachment_path(file_path: str, ctx: ToolContext) -> str | None:
    """Return resolved path when it is one of this turn's uploaded attachments."""
    try:
...
class ReadTool(ToolDefinition):
    ...
    def _read_data_file(self, file_path: str, ext: str, ctx: ToolContext) -> ToolResult:
        ...
        try:
            ...
            resolved = _is_allowed_attachment_path(file_path, ctx)
            if resolved is None:
            ...
            if resolved is None:
                return ToolResult(
                    error=(
                        ...
                        "and was not attached to this request"
                    )
                )
    ...
    async def _filesystem_execute(self, args: dict[str, Any], ctx: ToolContext) -> ToolResult:
        ...
        try:
            ...
            allowed_path = _is_allowed_attachment_path(file_path, ctx)
