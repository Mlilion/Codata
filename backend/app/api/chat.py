...
def _is_expert_team_creation_request(body: PromptRequest | EditAndResendRequest) -> bool:
    skills = {str(skill).strip() for skill in (body.skills or []) if str(skill).strip()}
    return body.mode == "expert_team_creation" or "create_expert_teams" in skills
...
async def start_prompt(
    ...
    if _is_expert_team_creation_request(body):
        ...
        body.provider_id = access.provider_id
