...
def _extract_prompt(messages: list[ChatMessage]) -> tuple[str, str | None]:
    ...
    Channel bridge metadata is discarded to avoid wasting tokens and confusing
    the agent.
    """
    user_text = ""
    ...
    # Strip channel metadata that some bridge clients embed directly in the
    # user message content.
    user_text = _strip_channel_metadata(user_text)

    return user_text, None


def _strip_channel_metadata(text: str) -> str:
    """Remove channel bridge metadata embedded in user message text.

    Some bridge clients send messages like:
      System: [2026-03-26 17:00:46 EDT] gateway connected as +1234567890.
...
def _extract_sender_info(messages: list[ChatMessage]) -> dict[str, str | None]:
    """Extract sender name and other metadata from channel system messages.

    Channel bridge clients can include metadata messages like:
      Conversation info (untrusted metadata):
