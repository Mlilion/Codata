...
async def _complete_oauth_flow_internal(code: str, state: str) -> str:
    ...
    _update_env_file("WORKCRAFT_OPENAI_OAUTH_ACCESS_TOKEN", access_token)
    _update_env_file("WORKCRAFT_OPENAI_OAUTH_REFRESH_TOKEN", refresh_token)
    _update_env_file("WORKCRAFT_OPENAI_OAUTH_ACCOUNT_ID", account_id)
    _update_env_file("WORKCRAFT_OPENAI_OAUTH_EXPIRES_AT", str(expires_at_ms))
    _update_env_file("WORKCRAFT_OPENAI_OAUTH_EMAIL", email)
...
async def disconnect_openai_subscription(settings: SettingsDep, registry: ProviderRegistryDep) -> OpenAISubscriptionStatus:
    ...
    # Remove from .env
    _remove_env_key("WORKCRAFT_OPENAI_OAUTH_ACCESS_TOKEN")
    _remove_env_key("WORKCRAFT_OPENAI_OAUTH_REFRESH_TOKEN")
    _remove_env_key("WORKCRAFT_OPENAI_OAUTH_ACCOUNT_ID")
    _remove_env_key("WORKCRAFT_OPENAI_OAUTH_EXPIRES_AT")
    _remove_env_key("WORKCRAFT_OPENAI_OAUTH_EMAIL")
...
<html>
<head><title>WorkCraft — Authentication Successful</title></head>
<body style="font-family: -apple-system, system-ui, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; background: #0a0a0a; color: #e0e0e0;">
  <div style="text-align: center; max-width: 400px; padding: 2rem;">
    ...
    <p style="color: #888; font-size: 0.875rem;">Signed in{display}. ChatGPT subscription models are now available in WorkCraft.</p>
...
def _error_html(message: str) -> str:
    return f"""<!DOCTYPE html>
<html>
<head><title>WorkCraft — Authentication Failed</title></head>
<body style="font-family: -apple-system, system-ui, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; background: #0a0a0a; color: #e0e0e0;">
  <div style="text-align: center; max-width: 400px; padding: 2rem;">
    <div style="font-size: 3rem; margin-bottom: 1rem;">&#10007;</div>
