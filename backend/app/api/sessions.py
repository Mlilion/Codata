...
async def _session_responses_with_expert_slugs(
    db: AsyncSession,
    sessions: list[Session],
) -> list[SessionResponse]:
    responses = [SessionResponse.model_validate(session) for session in sessions]
    missing_ids = [
        response.id
    ...
    message_result = await db.execute(
        ...
        .where(Message.session_id.in_(missing_ids))
        .where(
            (Message.data["agent"].as_string() == "expert-team")
            | (Message.data["mode"].as_string() == "expert-team")
        )
    )
    expert_slugs: dict[str, str] = {}
    for session_id, data in message_result.all():
        if session_id in expert_slugs:
            continue
        if not isinstance(data, dict):
            continue
        team_id = str(data.get("expert_team") or "unknown").strip() or "unknown"
        expert_slugs[session_id] = f"expert-team:{team_id}"

    unresolved_ids = [session_id for session_id in missing_ids if session_id not in expert_slugs]
    if unresolved_ids:
        part_result = await db.execute(
            select(Part.session_id, Part.data)
            .where(Part.session_id.in_(unresolved_ids))
            .where(Part.data["type"].as_string().in_(["step-start", "step-finish"]))
            .where(Part.data["snapshot"]["mode"].as_string() == "expert-team")
        )
        for session_id, data in part_result.all():
            if session_id in expert_slugs:
                continue
            if not isinstance(data, dict):
                continue
            ...
            if not isinstance(snapshot, dict):
                continue
            team_id = str(snapshot.get("expert_team") or "unknown").strip() or "unknown"
            expert_slugs[session_id] = f"expert-team:{team_id}"

    if not expert_slugs:
        return responses
    return [
        response.model_copy(update={"slug": expert_slugs[response.id]})
        if response.id in expert_slugs and not response.slug
        else response
        for response in responses
    ]


async def _session_response_with_expert_slug(db: AsyncSession, session: Session) -> SessionResponse:
    return (await _session_responses_with_expert_slugs(db, [session]))[0]


async def _presented_file_paths(db: AsyncSession, session_id: str) -> set[str]:
    """Collect file paths explicitly shown through present_file."""
    result = await db.execute(
        select(Part.data)
        .where(Part.session_id == session_id)
        .where(Part.data["type"].as_string() == "tool")
        .where(Part.data["tool"].as_string() == "present_file")
    )
    paths: set[str] = set()
    for data in result.scalars().all():
        ...
        candidates: list[str] = []
        for value in (
            state.get("metadata").get("file_path") if isinstance(state.get("metadata"), dict) else None,
            state.get("input").get("file_path") if isinstance(state.get("input"), dict) else None,
        ):
            if isinstance(value, str) and value.strip():
                candidates.append(value)
        output = state.get("output")
        if isinstance(output, str) and output.strip():
            candidates.extend(_PATH_PATTERN.findall(output))
...
async def _file_part_metadata_by_path(db: AsyncSession, session_id: str) -> dict[str, dict]:
    """Return persisted file-part metadata keyed by resolved path."""
    result = await db.execute(
        select(Part.data)
        .where(Part.session_id == session_id)
        .where(Part.data["type"].as_string() == "file")
    )
...
def _file_visibility(
    tool_id: str,
    file_path: str,
    presented_paths: set[str],
    file_part_metadata: dict | None = None,
) -> FileVisibility:
    ...
    if tool_id in _DRAFT_TOOLS:
        return "draft"
    if tool_id in _INTERMEDIATE_TOOLS:
        return "intermediate"
    return "hidden"
...
async def search_sessions_endpoint(
    ...
    response_by_id = {item.id: item for item in await _session_responses_with_expert_slugs(db, sessions)}
    return [
        SessionSearchResult(
            session=response_by_id.get(r["session"].id) or SessionResponse.model_validate(r["session"]),
            snippet=r["snippet"],
        )
        for r in results
    ]
...
async def get_session_endpoint(
    ...
    return await _session_response_with_expert_slug(db, session)
...
async def get_session_files(
    ...
    if session is None or not session.directory:
        return {"files": [], "deliverable_files": []}

    presented_paths = await _presented_file_paths(db, session_id)
    file_part_metadata = await _file_part_metadata_by_path(db, session_id)
    ...
    deliverable_files: list[dict[str, object]] = []
    ...
    for entry in tracked_files:
        ...
        visibility = _file_visibility(entry.tool_id, resolved, presented_paths, file_part_metadata.get(resolved))
        tool_id = "present_file" if resolved in presented_paths else entry.tool_id
        payload = {
    ...
    for resolved in sorted(presented_paths):
        if resolved in seen_paths or not Path(resolved).is_file():
            continue
        seen_paths.add(resolved)
        payload = {
