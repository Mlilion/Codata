...
try:
    from app.dependencies import get_expert_role_registry, get_expert_team_registry, get_skill_registry
except Exception:  # pragma: no cover - dependency module is available in app runtime
    get_expert_role_registry = None  # type: ignore[assignment]
    get_expert_team_registry = None  # type: ignore[assignment]
...
def _normalize_selected_skills(skills: list[str] | None) -> list[str]:
    """Return selected skill names with empty and duplicate values removed."""
    if not skills:
        return []
...
def _is_expert_team_creation_mode(request: PromptRequest) -> bool:
    return request.mode == "expert_team_creation" or "create_expert_teams" in _normalize_selected_skills(request.skills)
...
def _allowed_attachment_paths(attachments: list[dict[str, Any]] | None) -> set[str]:
    allowed: set[str] = set()
    for att in attachments or []:
        ...
        try:
            ...
            logger.debug("Failed to resolve attachment path: %s", path, exc_info=True)
    return allowed


def _try_get_expert_team_registry() -> Any | None:
    if get_expert_team_registry is None:
        return None
    try:
        return get_expert_team_registry()
    except Exception:
        return None


def _try_get_expert_role_registry() -> Any | None:
    if get_expert_role_registry is None:
        return None
    try:
        return get_expert_role_registry()
    except Exception:
        return None


def _try_get_skill_registry() -> Any | None:
...
class SessionPrompt:
    ...
    def __init__(
        ...
        *,
        ...
        self.expert_team_registry: Any | None = None
        self.expert_role_registry: Any | None = None
    ...
    async def _setup(self) -> None:
        ...
        self.skill_registry = _try_get_skill_registry()
        ...
        if not model_id:
            ...
            candidates = [
                _m for _m in all_models
                if not provider_id or _m.provider_id == provider_id
            ]
            if not candidates:
                try:
        ...
        if self.skip_user_message:
            ...
            async with self.session_factory() as db:
                async with db.begin():
                    ...
                    if _is_expert_team_creation_mode(self.request):
                        instruction_msg = await create_message(
                            db,
                            session_id=session.id,
                            data={"role": "user", "agent": self.agent.name, "system": True, "hidden": True},
                        )
                        await create_part(
                            db,
                            message_id=instruction_msg.id,
                            session_id=session.id,
                            data={"type": "text", "text": _expert_team_creation_instruction(self.request.text)},
                        )
        ...
        await self._preload_selected_skills()
    ...
    async def _preload_selected_skills(self) -> None:
        """Load user-selected skills into hidden conversation context."""
        if self.skip_user_message:
            return
        selected = _normalize_selected_skills(self.request.skills)
        if not selected or self.agent is None:
            return

        skill_tool = self.tool_registry.get("skill")
        if skill_tool is None:
            logger.warning("Selected skills ignored because skill tool is unavailable: %s", selected)
            return

        async with self.session_factory() as db:
            async with db.begin():
                preload_msg = await create_message(
                    db,
                    session_id=self.job.session_id,
                    data={"role": "assistant", "agent": self.agent.name, "system": True, "hidden": True},
                )
                message_id = preload_msg.id
        ...
        for skill_name in selected:
            ...
            ctx = ToolContext(
                ...
                workspace=self.workspace,
                allowed_file_paths=_allowed_attachment_paths(self.request.attachments),
                index_manager=self.index_manager,
                messages=[],
                discovered_tools=self.discovered_tools,
                _publish_fn=lambda _event, _data: None,
            )
            ctx._app_state = {  # type: ignore[attr-defined]
                ...
                "expert_team_registry": self.expert_team_registry,
                "expert_role_registry": self.expert_role_registry,
                "skill_registry": self.skill_registry,
                "settings": _cfg(),
            }
            ctx._model_id = self.model_id  # type: ignore[attr-defined]
            ctx._provider_id = self.provider.id  # type: ignore[attr-defined]
            ctx._job = self.job  # type: ignore[attr-defined]
            ctx._depth = self.job._depth  # type: ignore[attr-defined]

            try:
                result = await skill_tool({"name": skill_name}, ctx)
            except Exception as exc:
                logger.warning("Failed to preload selected skill %s", skill_name, exc_info=True)
                output = f"Failed to load selected skill {skill_name}: {exc}"
                title = f"Skill load failed: {skill_name}"
                status = "error"
                metadata: dict[str, Any] | None = {"name": skill_name}
            else:
                output = result.output or result.error or ""
                title = result.title or f"Loaded skill: {skill_name}"
