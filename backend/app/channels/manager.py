...
Adapted for WorkCraft: uses ChannelsConfig instead of nanobot Config,
...
class ChannelManager:
    ...
    Responsibilities:
    - Initialize enabled channels (DingTalk, Feishu, Telegram, etc.)
    - Start/stop channels
    ...
    def add_channel(self, name: str, channel: BaseChannel) -> None:
        """Register a channel without starting it."""
        self.channels[name] = channel
    ...
    def _ensure_dispatcher(self) -> None:
        """Start the outbound dispatcher if it is not already running."""
        if self._dispatch_task is None or self._dispatch_task.done():
            self._dispatch_task = asyncio.create_task(
                self._dispatch_outbound(),
                name="channel-outbound-dispatcher",
            )

    async def add_and_start_channel(self, name: str, channel: BaseChannel) -> None:
        """Dynamically add and start a channel while the app is running."""
        old_channel = self.channels.get(name)
        old_task = self._channel_tasks.pop(name, None)
        ...
        if old_channel:
            try:
                ...
                logger.warning("Error stopping existing %s channel: %s", name, e)
