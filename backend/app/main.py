...
from app.dependencies import (
    ...
    get_expert_team_registry,
    set_agent_registry,
    set_connector_registry,
    set_expert_role_registry,
    set_expert_team_registry,
    set_index_manager,
    set_plugin_manager,
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # Runtime CSRF allowlist. Kept for wrappers that need to add origins
    # dynamically; remote tunnel management is no longer exposed.
    ...
    # Expert team registry (preset teams + user/project YAML)
    from app.expert.registry import ExpertTeamRegistry
    from app.expert.roles import ExpertRoleRegistry

    expert_team_registry = ExpertTeamRegistry(
        project_dir=settings.project_dir,
        remote_enabled=settings.remote_expert_teams_enabled,
        remote_manifest_path=settings.remote_expert_teams_manifest_path or None,
        remote_manifest_url=settings.remote_expert_teams_manifest_url,
        remote_auth_token=settings.remote_expert_teams_auth_token,
        remote_cache_path=settings.remote_expert_teams_cache_path or None,
        remote_fetch_interval_seconds=settings.remote_expert_teams_fetch_interval_seconds,
    )
    expert_team_registry.scan()
    app.state.expert_team_registry = expert_team_registry
    set_expert_team_registry(expert_team_registry)

    expert_role_registry = ExpertRoleRegistry(project_dir=settings.project_dir)
    expert_role_registry.scan()
    app.state.expert_role_registry = expert_role_registry
    set_expert_role_registry(expert_role_registry)
    ...
    _register_builtin_tools(
        ...
        skill_registry=skill_registry,
        settings=settings,
        expert_team_registry=expert_team_registry,
        expert_role_registry=expert_role_registry,
    )
    ...
    channels_config_path = resolve_channels_config_path(settings.channels_config_path)
    channels_config = load_channels_config(channels_config_path)
    ...
    app.state.channels_config_path = channels_config_path
...
def _register_builtin_tools(
    ...
    *,
    ...
    create_expert_tool = registry.get("create_expert_teams")
    if create_expert_tool is not None:
        if expert_team_registry is None:
            try:
                expert_team_registry = get_expert_team_registry()
            except Exception:
                expert_team_registry = None
        if expert_role_registry is None:
            try:
                expert_role_registry = get_expert_role_registry()
            except Exception:
                expert_role_registry = None
        if expert_team_registry is not None:
            setattr(create_expert_tool, "_expert_team_registry", expert_team_registry)
        if expert_role_registry is not None:
            setattr(create_expert_tool, "_expert_role_registry", expert_role_registry)
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    # Serve root frontend static files used by the backend bundle.
    # In desktop mode, Tauri serves the main frontend from its own resources.
