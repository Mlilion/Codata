...
async def refresh_proxy_token(
    ...
    async with _refresh_lock:
        try:
            ...
            # Persist to .env
            _update_env_file("WORKCRAFT_PROXY_TOKEN", new_access)
            if new_refresh:
                _update_env_file("WORKCRAFT_PROXY_REFRESH_TOKEN", new_refresh)

            # Update the live provider's API key
            provider = getattr(provider_registry, "get_provider", lambda _: None)("workcraft-proxy")
