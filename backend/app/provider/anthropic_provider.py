...
Delegates to the yakagent AnthropicProvider (native Anthropic SDK with full
Claude feature support: extended thinking, prompt caching, native tools)
and converts between upstream and desktop schema types.
"""
...
def _convert_model_info(m: Any) -> ModelInfo:
    """Convert an upstream ModelInfo to the desktop schema."""
    return ModelInfo(
        id=m.id,
        name=m.name,
...
class AnthropicDesktopProvider(BaseProvider):
    ...
    Wraps the yakagent native AnthropicProvider to get full Claude support
    ...
    async def list_models(self) -> list[ModelInfo]:
        ...
        # Merge with the bundled fallback catalog to fill gaps
        # (models.dev might miss latest models; the fallback may have them)
