...
│   ├── proxy_auth.py    #   WorkCraft Cloud 代理认证
...
| | **消息渠道** | |
| GET | `/api/channels` | 列出已配置的消息渠道 |
| GET | `/api/channels/status` | 渠道运行状态 |
| POST | `/api/channels/login` | 开始支持渠道的交互式登录 |
...
| `WORKCRAFT_PROXY_URL` | WorkCraft Cloud 代理地址（计费模式） | `` |
| `WORKCRAFT_PROXY_TOKEN` | 代理认证 JWT | `` |
